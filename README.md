...
1. Download `Progressy-1.2.0-arm64.dmg` from the [releases](../../releases) (or build it yourself, see below).
...
1. Download and run `Progressy Setup 1.2.0.exe`, or use the portable `.exe` if you would rather not
   install anything.
...
```bash
chmod +x Progressy-1.2.0.AppImage && ./Progressy-1.2.0.AppImage
# or, on Debian/Ubuntu:
sudo dpkg -i progressy_1.2.0_amd64.deb
```
